...
import pandas as pd
from queries.queryMstOutlet import *
...
def main():
    ...
    #fetching data
    result = view_all_data_master_outlet()
    ...
    page_size = st.selectbox("Rows per page", [10, 20, 30, 50, 100], index=2, key="rows_mstoutlet")  # Default 30
    ...
    with col1:
        if st.button("Previous",key="previous_button_mstoutlet") and st.session_state.page_number > 1:
            st.session_state.page_number -= 1
    with col2:
        st.write(f"Page {st.session_state.page_number} of {total_pages}")
    with col3:
        if st.button("Next",key="next_button_mstoutlet") and st.session_state.page_number < total_pages:
            st.session_state.page_number += 1
    ...
    st.write(f"Showing {start_row + 1}-{min(end_row, len(df_selection))} of {len(df_selection)}")
            ...
            # Export to Excel
    if st.button("Export ke Excel",key="export_excel_mstoutlet"):
        output = BytesIO()
